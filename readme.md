...
* **Apps to sleep** — the applications that put NVDA to sleep. Empty to begin with.
...
* **Running apps** — the applications running right now that are not on the list yet.
...
Both lists take more than one selection at a time, in the usual way: shift with the arrow keys extends the selection, control with the arrow keys and space picks items out one by one.

Nothing is changed until you press OK or Apply; Cancel leaves your settings exactly as they were.

## Notes

* Applications are listed under the name NVDA knows them by, which is the name of the executable without its extension — `firefox`, `notepad`, `explorer`. That is the name the add-on matches against, so what you see in the list is what is being compared.
