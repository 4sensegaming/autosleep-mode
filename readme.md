...
* **Apps to sleep** — the applications that put NVDA to sleep. Empty to begin with. An application that is running appears under the title of its window, just as it does in **Available apps** below; one that is not running has no window to take a title from, and appears under the name of its executable.
...
* **Available apps** — the applications running right now that are not on the list yet, each under the title of its window. Only applications with a window of their own are here, the same ones the task switcher offers you, so the background processes that never put anything on the screen stay out of the way. An application whose window carries no title, as a full screen self-voicing game may well not, is listed under the name of its executable instead.
* **Add** — puts the application you are on into the list. If you have selected several, the button becomes **Add selected** and puts all of them in at once. An application you add disappears from **Available apps**, and comes back to it if you remove it from **Apps to sleep** again.
* **Add apps with manually activated sleep mode to the autosleep list** — off by default. With it on, every application you put to sleep by hand with NVDA+shift+s joins the list, so it will sleep by itself from then on.
...
An application NVDA is asleep in as the dialog is drawn is shown with **(sleeping)** after its name, in whichever of the two lists it appears in. In **Apps to sleep** that is usually the add-on's own work reported back to you. In **Available apps** it is an application you have put to sleep by hand with NVDA+shift+s and have not added to the list, which is worth seeing if adding it is what you meant to do — particularly with **Add apps with manually activated sleep mode to the autosleep list** switched off, where nothing else would tell you. The mark is a snapshot taken as the category is opened, and does not change while the dialog is in front of you.
...
* A handful of applications are shown as sleeping without your ever having put them to sleep. NVDA ships app modules for a few self-voicing programs which switch sleep mode on by themselves, and the mark reports the state honestly rather than reporting only what you or the add-on did. There is no need to add such an application to **Apps to sleep**; it is quiet already.
* NVDA itself is deliberately left out of **Available apps**. Putting NVDA to sleep inside its own windows would only silence its own interface.
